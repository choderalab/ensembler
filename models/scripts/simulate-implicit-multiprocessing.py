...
#process_only_these_templates = ['SRC_HUMAN_P12931_PK0_2H8H_A', 'HCK_HUMAN_P08631_PK0_1AD5_A', 'ABL1_HUMAN_P00519_PK0_2HZ0_A', 'CDK2_HUMAN_P24941_PK0_1KE7_A']
...
def simulate_implicit(model_directory, variants=None, gpuid=0):
        print "-------------------------------------------------------------------------"
        print "gpuid %d directory %s" % (gpuid, model_directory)
        print "-------------------------------------------------------------------------"
...
def worker(queue, reject_queue, gpuid):
    print 'Worker %d started' % gpuid
    while True:
        item = queue.get()
        if item is None: # detect sentinel
            break
        try:
            [model_directory, variants] = item
            simulate_implicit(model_directory, variants, gpuid)
        except Exception as e:
            # Record rejected models.
            reason = str(e)
            reject_queue.put((item, reason))
            print "Worker %d encountered exception due to reason: %s" % (gpuid, reason)
            import traceback
            print traceback.format_exc()
        queue.task_done()
    print 'Worker %d finished' % gpuid
    queue.task_done()
...
reject_queue = multiprocessing.JoinableQueue()

# Create a worker pool.
pool = list()
for gpuid in range(ngpus):    
    process = multiprocessing.Process(target=worker, args=(queue, reject_queue, gpuid))
    process.daemon = True
    process.start()
    pool.append(process)
...
for target in targets:
    ...
    for template in templates:
        ...
        # Add to work queue.
        queue.put((model_directory, variants))
  
# Add termination sentinels to the queue.
for gpuid in range(ngpus):
    queue.put(None)

# Wait for work to be completed.
import time
while not queue.empty(): # wait for processing to finish
    time.sleep(1)   # manage timeouts and status updates etc.

# Check for rejects before terminating.
reject_filename = os.path.join(models_directory, 'reject-implicit.txt')
reject_file = open(reject_filename, 'w')
while not reject_queue.empty():
    (model_directory, reason) = reject_queue.get()
    reject_file.write('%s : %s\n' % (model_directory, reason))    
reject_file.close()
