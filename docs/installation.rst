...
.. warning:: For Modeller to work properly, both the Modeller Python library and compiled library file ``_modeller.so`` must be available to your Python installation. To test this (after installing according to the instructions on the Modeller site), try typing ``import modeller`` in a Python interpreter. If either of these libraries is not available to your Python installation, then you will see an error message saying that either ``modeller`` or ``_modeller`` have not been found. A simple way to resolve this is to set your PYTHONPATH environment variable with the appropriate paths. For example, on an OS X machine with a bash shell, you would type: ``export PYTHONPATH=/Library/modeller-9.12/modlib/:/Library/modeller-9.12/lib/mac10v4:${PYTHONPATH}`` (substitute the correct Modeller installation directory as appropriate). Try ``import modeller`` in your Python interpreter again, and once it is working correctly, add the ``export PYTHONPATH=`` command to your shell configuration file (e.g. ``.bashrc`` or ``.bash_profile``). On a Linux machine, the appropriate Modeller installation directories would likely be similar to ``/home/travis/bin/modeller9.14/modlib`` and ``/home/travis/bin/modeller9.14/lib/x86_64-intel8/python2.5``
